...
import org.move.lang.core.psi.ext.isPublicPackage

class MvReplaceWithPackageVisInspection:
    ...
    override fun visitTargetElement(element: MvFunction, holder: ProblemsHolder, isOnTheFly: Boolean) {
        ...
        if (!visModifier.isPublicPackage) return
        holder.registerProblem(
            visModifier,
            "`public(package)` can be replaced with `package`",
            WEAK_WARNING,
            ReplaceWithPackageFix(visModifier)
        )
    }

    class ReplaceWithPackageFix(visModifier: MvVisibilityModifier):
        DiagnosticFix<MvVisibilityModifier>(visModifier) {

        override fun getText(): String = "Replace `public(package)` with `package`"
